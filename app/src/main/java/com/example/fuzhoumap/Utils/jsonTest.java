...
//		   });
			...
			ArrayList<BaiduResponse.Results> list3 = new ArrayList<BaiduResponse.Results>();
				   ...
				   for(BaiduResponse.Results item:list) {
					   if(item.price!=null) {
						   if(Double.valueOf(item.price)<=50.0) {
							   list0.add(item);
							   //if(list0.size()==5) break;
						   }
						   else if(Double.valueOf(item.price)>50.0&&Double.valueOf(item.price)<=100.0) {
							   list1.add(item);
						   }
						   else if(Double.valueOf(item.price)>100.0&&Double.valueOf(item.price)<=200.0) {
							   list2.add(item);
						   }
						   else if(Double.valueOf(item.price)>200.0) {
							   list3.add(item);
						   }
					   }
				   }
